...
describe('Injector — инъекция глазика на реальной разметке ленты', () => {
  ...
  it('картинка-превью (js-image-resizer) получает глазик с kind=image', () => {
    mountFeed(
      '<div class="feed-note-wrapper feed-note-wrapper-note"><div class="feed-note__media-preview">' +
      '<a class="js-image-resizer feed-note__media-preview__link" href="https://venskons78.amocrm.ru/download/drive/aa/bb">' +
      '<img class="feed-note__media-preview__picture" alt="photo.jpg" src="https://venskons78.amocrm.ru/download/drive/aa/bb"/>' +
      '</a></div></div>'
    );
    const onEyeClick = vi.fn();
    const inj = new Injector({ $: jq, onEyeClick });
    ...
    expect(eye.getAttribute('data-kind')).toBe('image');
    expect(eye.getAttribute('data-name')).toBe('photo.jpg');
    eye.dispatchEvent(new window.MouseEvent('click', { bubbles: true }));
    expect(onEyeClick).toHaveBeenCalledOnce();
    expect(onEyeClick.mock.calls[0][0].kind).toBe('image');
    inj.stop();
  });

  it('тултип глазика берётся из langs (локализация, без хардкода)', () => {
    mountFeed(NOTE_DOCX);
    const inj = new Injector({
      $: jq, onEyeClick: () => {},
      langs: { widget: { eye: { tooltip: 'Предпросмотр' } } }
    });
    inj.start();
    expect(document.querySelector('.nx-eye').getAttribute('title')).toBe('Предпросмотр');
    inj.stop();
  });

  it('без langs тултип — английский фолбэк, не русский', () => {
    mountFeed(NOTE_DOCX);
    const inj = new Injector({ $: jq, onEyeClick: () => {} });
    inj.start();
    expect(document.querySelector('.nx-eye').getAttribute('title')).toBe('Preview');
    inj.stop();
  });

  it('картинка без alt/title получает локализованное дефолтное имя', () => {
    mountFeed(
      '<div class="feed-note__media-preview">' +
      '<a class="js-image-resizer" href="https://venskons78.amocrm.ru/download/drive/aa/bb">' +
      '<img src="https://venskons78.amocrm.ru/download/drive/aa/bb"/>' +
      '</a></div>'
    );
    const inj = new Injector({
      $: jq, onEyeClick: () => {},
      langs: { widget: { image: { default_name: 'Изображение' } } }
    });
    inj.start();
    expect(document.querySelector('.nx-eye').getAttribute('data-name')).toBe('Изображение');
    inj.stop();
  });
